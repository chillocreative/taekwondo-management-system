...
export default function CoachAttendanceIndex({ auth, trainingCenter, students, date, flash }) {
    ...
    useEffect(() => {
        ...
        students.forEach(student => {
            initialStates[student.id] = {
                status: student.status || 'hadir', // Default to 'hadir' if not set? Or null? 
                // Requirement doesn't specify default. 'Hadir' is usually safe for quick entry.
                // If it's NULL (not marked), let's default to 'hadir' so they can just click "Save" for everyone present.
                // Or maybe default to null to force check? 
                // "Hadir" default is faster for coaches.
    ...
    const handleDateChange = (e) => {
        ...
        router.get(route('coach.attendance.index'), { date: newDate }, { preserveState: true, preserveScroll: true });
    };
