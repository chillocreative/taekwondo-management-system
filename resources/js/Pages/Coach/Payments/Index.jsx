...
export default function CoachPaymentsIndex({ auth, payments, filters, trainingCenters }) {
    ...
    useEffect(() => {
        const timeoutId = setTimeout(() => {
            if (search !== (filters?.search || '') || tcId !== (filters?.training_center_id || '')) {
                router.get(route('coach.payments.index'), {
                    ...(filters || {}),
                    search: search,
    ...
    return (
        <AuthenticatedLayout
            user={auth?.user}
            header={<h2 className="font-bold text-2xl text-gray-800 leading-tight">Senarai Pembayaran</h2>}
        >
                                ...
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {payments?.data && payments.data.length > 0 ? (
                                        payments.data.map((payment) => (
                                            <tr key={payment?.id || Math.random()} className="hover:bg-blue-50/50 transition duration-150">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full ${payment?.status === 'paid' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                                                        }`}>
                                        ...
                                        <tr>
                                            <td colSpan="7" className="px-6 py-12 text-center text-gray-500">
                                                <div className="flex flex-col items-center gap-2">
                                                    ...
                                                    <p className="font-medium">Tiada rekod pembayaran dijumpai.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
